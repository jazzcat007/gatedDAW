...
- **Above 25 ms** — noticeably delayed; playing live instruments feels "rubbery", and gatedDAW shows a warning
...
- **Interfaces with direct monitoring**: almost all of the above have a knob or switch that routes the microphone/instrument input straight to your headphones inside the hardware, with **zero** latency. For recording vocals, use direct monitoring for your own voice and let gatedDAW play the backing — the round-trip latency stops mattering entirely
...
- Chrome and Chromium-based browsers currently report and achieve the lowest, most stable output latency and are the recommended way to run gatedDAW
- Close tabs that also play or capture audio (calls, videos): they can hold the audio device at a higher buffering mode
- Plug in your output device **before** loading gatedDAW; switching devices mid-session can leave the browser on a conservative buffer until reload
